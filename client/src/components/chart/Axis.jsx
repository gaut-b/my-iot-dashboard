...
import { dimensionsPropsType } from "./utils/useChartDimensions";

const AxisHorizontal = ({ dimensions, label, formatTick, scale, ...props }) => {
  ...
  return (
    <g className="Axis AxisHorizontal" transform={`translate(0, ${dimensions.boundedHeight})`} {...props}>
      ...
      {label && (
          <text className = "Axis__label" transform={`translate(${dimensions.boundedWidth / 2}, 45)`}>
            { label }
          </text>
      )}
    </g>
...
const AxisVertical = ({ dimensions, label, formatTick, scale, ...props }) => {
  ...
  return (
    <g className="Axis AxisVertical" {...props}>
      ...
      {ticks.map((tick, i) => (
        <text key={tick} className="Axis__tick" transform={`translate(-20, ${scale(tick)})`} >
          { formatTick(tick) }
        </text>
      ))}

      {label && (
        <text className="Axis__label" style={{
